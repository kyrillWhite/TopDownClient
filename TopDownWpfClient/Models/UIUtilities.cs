...
namespace TopDownWpfClient.Models {
    public class UIUtilities {
        private const double SCROLL_BAR_THICKNESS = 18;
        ...
        public static T FindDescendant<T>(DependencyObject element, Func<T, bool> searchFunc = null)
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                var childResult = FindDescendant<T>(child, searchFunc);
                if (childResult != null) {
                    return childResult;
                }
            }
        ...
        public static void BringIntoView(FrameworkElement element) {
            ...
            var scrollViewer = UIUtilities.FindVisualParent<ScrollViewer>(element);
            ...
            if (scrollViewer.Content is not FrameworkElement scrollViewerContent) {
                return;
            }

            GeneralTransform transform = element.TransformToVisual(scrollViewerContent);

            Point positionInScrollViewer = transform.Transform(new Point(0, 0));
            ...
            } else if (itemEnd > currentViewEnd - SCROLL_BAR_THICKNESS) {
                if (element.ActualHeight > viewerHeight - SCROLL_BAR_THICKNESS) {
                    scrollViewer.ScrollToVerticalOffset(itemStart);
                } else {
                    scrollViewer.ScrollToVerticalOffset(itemEnd - viewerHeight + SCROLL_BAR_THICKNESS);
                }
            }
        }
    }
